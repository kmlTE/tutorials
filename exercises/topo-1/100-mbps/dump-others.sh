...
for i in $(seq 1 3); do
    ...
    echo $filename
    echo 0 | nl -b a -v 0 | tee $filename
    cat ${algo_dir}/${next_dir}/raw/udp/h4-iperf.csv | grep -v "0.0-${iperf_time}" | grep ",10.0.1.${i}," | cut -d ',' -f 13 | nl -b a -i 1 -v 1 | tee -a $filename
done
...
for i in $(seq 1 3); do
    ...
    echo $filename
    echo 0 | nl -b a -v 0 | tee $filename
    cat ${algo_dir}/${next_dir}/raw/udp/h4-iperf.csv | grep -v "0.0-${iperf_time}" | grep ",10.0.1.${i}," | cut -d ',' -f 10 | nl -b a -i 1 -v 1 | tee -a $filename
done
...
for i in $(seq 1 3); do
    ...
    echo $filename
    echo 0 | nl -b a -v 0 | tee $filename
    cat ${algo_dir}/${next_dir}/raw/udp/h4-iperf.csv | grep -v "0.0-${iperf_time}" | grep ",10.0.1.${i}," | cut -d ',' -f 15 | nl -b a -i 1 -v 1 | tee -a $filename
done