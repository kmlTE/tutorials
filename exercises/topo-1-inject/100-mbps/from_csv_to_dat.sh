...
for i in $(seq 2 3); do
	...
	cat ${algo_dir}/${next_dir}/raw/tcp/h4-iperf.csv | grep -v "0.0-${iperf_time}" | grep ",10.0.1.${i}," | cut -d ',' -f 9 | nl -b a -i 1 -v 1 | tee -a $filename
done
...
echo $filename
yes "0" | head -n $(( $iperf_period + 1 )) | nl -b a -v 0 | tee $filename
cat ${algo_dir}/${next_dir}/raw/udp/h4-iperf.csv | grep -v "0.0-${iperf_period}" | grep ",10.0.1.1," | cut -d ',' -f 9 | nl -b a -i 1 -v $(( $iperf_period + 1 )) | tee -a $filename
yes "0" | head -n $iperf_period | nl -b a -v $(( $iperf_period * 2 + 1 )) | tee -a $filename
...
for i in $(seq 2 3); do
	...
	echo $filename
	echo 0 | nl -b a -v 0 | tee $filename
	cat ${algo_dir}/${next_dir}/raw/udp/h4-iperf.csv | grep -v "0.0-${iperf_time}" | grep ",10.0.1.${i}," | cut -d ',' -f 9 | nl -b a -i 1 -v 1 | tee -a $filename
done
