...
TOPO=topo-3
TOPO_DESCR="TOPO-3: simple tree (d=2)"

export ROOT_DIR=$(pwd)

export LINK_CAP_MB=10
export AGR_LINK_CAP_MB=$(( $LINK_CAP_MB * 2 ))
...
export AGR_LINK_RATE=$(( $LINK_RATE * 2 ))
...
run_single () {
	...
	for i in $(seq 1 7); do
		cat s${i}-runtime.json.tmpl | envsubst > s${i}-runtime.json
	done;

	make clean && make run && make clean

	rm ../run_iperf topology.json
...
iperf_def () {
	...
	export HP_RATE=$FAIR_RATE
	export LP_RATE=$FAIR_RATE
	export HP_QUEUE=6
	iperf_toggle
}
